...
def h_atp_mi(cm_h, ci_h, z_h, vm, vi,  ami, parama_mi_h):
...
def nak_atp(ca_k, cb_k, ca_na, cb_na, cb_nh4, a, nak_atp_param):
    ...
    if nak_atp_param == 0:
        ...
        # sodium affinity
        knpn = 0.002 * (1.0 + ca_k / .00833)
        # potesium affinity
        knpk = 0.001 * (1.0 + cb_na / .0185)
...
def goldman(hab, a, z, va, vb, ca, cb, param_goldman):
    ...
    # [Normalized_PD] ----> [1] = [1]*[Joule/mV.mmol]*[mV]*[Joule/mmol]^-1
    # [Goldman_flux]----> [mmol/s] =[ml/s]*[mmol/ml]
...
def csf(ca, cb, flux, s, param_csf):
    # convective solute fluxes: water_flux*(1-sigma)*logarithmic mean solute concentration
    # [Convetive_flux]---->[mmol/s] = [ml/s]*[1]*[mmol/ml]
    if param_csf == 0:
        return 0
    ...
    # s is the reflection coefficient for the membrane ab with no unit
...
def matrix(init, h, tag):
    if tag == 'clever':
        ...
        b = 0.0e0
        c = 1.0e-6
    return[a + b * init + c for x in range(h)]


def f_eps(c, z, v):
    # electrochemical potential of species
    # see label {Electrochemical_P}
    # [xm]---->[Joule/ml] = [Joule/mmol][1] + [1][Joule/mV.mmol][mV]
    if c > 0 and c != 0:
...
def eQs(guess, solver):
    ...
    # Weinstein (2007)
    # see label {ES_area}
    aes[t] = ae0 * (1.0 + mua * (pe[t] - pm)) # [cm2/cm2.Epithelium]
    aes[t] = ae0 if aes[t] < ae0 else aes[t]
    # see label {LIS_volume}
    chvl[t] = chvl0 * (1.0 + muv * (pe[t] - pm)) # [cm3/cm2.Epithelium]
    ...
    # [water_flux]---->[ml/s] = [cm3.ml/mmol.s]*[mmol/mmHg.ml]*[mmHg] + [cm3.ml/mmol.s]*[mmol/ml]
    ...
    # (cm_na, ci_na, cm_gluc, ci_gluc, z_na, z_gluc, vm, vi, ami, lmi_nagluc, param_sglt_mi):
    ...
    # see label {eq: H_ATP}
    act_h_mi = h_atp_mi(cm_h, ci_h[t], z_h, vm[t], vi[t], ami, h_mi_atp_param)
    # updating the related flux on the apical membrane
    fikm_h = fikm_h + act_h_mi
...
aes = matrix(0.0200, T, tag)
chvl = matrix(0.700e-04, T, tag)
clvl = matrix(0.100e-02, T, tag)
l = matrix(0.100e-02, T, tag)
imp = matrix(0.600e-01, T, tag)
...
# tag = arbitrary//clever//clever0.25

print('tag',tag)
pltfolder = tag + '_'+ str(Nfac) + '_init_cond_figs'
...
if plot_concen:
    ...
    ax [ 1 ].plot(t, vm, 'b-', linewidth=2.0)
    ax [ 1 ].set_ylabel('voltage_m (mv) ', color='blue')
    ax [ 1 ].set_xlabel('time (s) ', color='blue')
    ax [ 1 ].set_xlim(-5, tf)
    ve = np.asarray(ve)

    ax [ 3 ].plot(t, ve, 'b-', linewidth=2.0)
    ax [ 3 ].set_ylabel('voltage_e (mv) ', color='blue')
    ...
    ax [ 4 ].plot(t, vi, 'b-', linewidth=2.0)
